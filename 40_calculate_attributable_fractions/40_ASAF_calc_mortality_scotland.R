...
# The aim of this code is to estimate the fractions of disease attributable to alcohol and tobacco - mortality
...
years <- 2008:2019
substance <- "alcohol_and_tobacco"
subtance_handel <- "tobalc"
date <- Sys.Date()
mort_or_morb <- "mort"
country <- "Scotland"
...
est <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = NULL, mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

# with tobacco-alcohol interaction
est_tobalc_int <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = NULL, tobalc_include_int = TRUE,
               mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

# merge estimates together
colnames(est_tobalc_int)[colnames(est_tobalc_int) == "af"] ="af_tobalc_int"
est <- merge(est, est_tobalc_int, by = c("year", "condition"))
...
setnames(est, "af_tobalc_int", "Population Attributable Fraction: tob-alc interaction")

stapmr::WriteToExcel(wb, sheet = "5 year Condition", 
                     title = "Table 1. Alcohol and Smoking Attributable Fractions for mortality. For a pooled sample of 2015 to 2019.", 
...
est_tobalc_int <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = c("ageband"), tobalc_include_int = TRUE,
               mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

# merge estimates together
colnames(est_tobalc_int)[colnames(est_tobalc_int) == "af"] ="af_tobalc_int"
est <- merge(est, est_tobalc_int, by = c("ageband", "year", "condition"))
...
stapmr::WriteToExcel(wb, sheet = "5 year Age band", 
                     title = "Table 2. Alcohol and Smoking Attributable Fractions for mortality by age-band. For a pooled sample of 2015 to 2019.", 
                     est, startCol = 1, startRow = 1)
...
est_tobalc_int <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = c("sex"), tobalc_include_int = TRUE,
               mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

# merge estimates together
colnames(est_tobalc_int)[colnames(est_tobalc_int) == "af"] ="af_tobalc_int"
est <- merge(est, est_tobalc_int, by = c("sex", "year", "condition"))
...
stapmr::WriteToExcel(wb, sheet = "5 year Sex", 
                     title = "Table 3. Alcohol and Smoking Attributable Fractions for mortality by sex. For a pooled sample of 2015 to 2019.", 
                     est, startCol = 1, startRow = 1)
...
est_tobalc_int <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = c("imd_quintile"), tobalc_include_int = TRUE, 
               mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

# merge estimates together
colnames(est_tobalc_int)[colnames(est_tobalc_int) == "af"] ="af_tobalc_int"
est <- merge(est, est_tobalc_int, by = c("imd_quintile", "year", "condition"))
...
stapmr::WriteToExcel(wb, sheet = "5 year IMD quintile", 
                     title = "Table 4. Alcohol and Smoking Attributable Fractions for mortality by Index of Multiple Deprivation quintiles. For a pooled sample of 2015 to 2019.", 
                     est, startCol = 1, startRow = 1)
...
est_tobalc_int <- PAFcalc(data = data, substance = subtance_handel, 
               tob_include_risk_in_former_smokers = TRUE, alc_protective = TRUE, use_weights = TRUE, 
               year_range = (max(years)-4):max(years), pool = TRUE, 
               subgroups = c("ageband", "sex", "imd_quintile"), tobalc_include_int = TRUE,
               mort_or_morb = mort_or_morb,
               oesoph_subtypes = TRUE)

colnames(est_tobalc_int)[colnames(est_tobalc_int) == "af"] ="af_tobalc_int"
est <- merge(est, est_tobalc_int, by = c("ageband", "sex", "imd_quintile", "year", "condition"))
...
stapmr::WriteToExcel(wb, sheet = "5 year Sex Age IMD", 
                     title = "Table 5. Alcohol and Smoking Attributable Fractions for mortality by sex, age-band and Index of Multiple Deprivation quintiles. For a pooled sample of 2015 to 2019.", 
                     est, startCol = 1, startRow = 1)
...
stapmr::WriteToExcel(wb, sheet = "Time series Condition", 
                     title = "Table 6. Alcohol and Smoking Attributable Fractions for mortality.", est, startCol = 1, startRow = 1)
...
stapmr::WriteToExcel(wb, sheet = "Time series Age band", 
                     title = "Table 7. Alcohol and Smoking Attributable Fractions for mortality by age-band.", 
                     est, startCol = 1, startRow = 1)
...
stapmr::WriteToExcel(wb, sheet = "Time series Sex", 
                     title = "Table 8. Alcohol and Smoking Attributable Fractions for mortality by sex.", 
                     est, startCol = 1, startRow = 1)
...
stapmr::WriteToExcel(wb, sheet = "Time series IMD quintile", 
                     title = "Table 9. Alcohol and Smoking Attributable Fractions for mortality by Index of Multiple Deprivation quintiles.", 
                     est, startCol = 1, startRow = 1)
...
stapmr::WriteToExcel(wb, sheet = "Time series Sex Age IMD", 
                     title = "Table 10. Alcohol and Smoking Attributable Fractions for mortality by sex, age-band and Index of Multiple Deprivation quintiles.", 
                     est, startCol = 1, startRow = 1)
